...
const NotificationsList = ({ setItemEdit, itemEdit }) => {
  ...
  const {
    ...
    queryKey: ["notifications", searchValue, store.isSearch, filterData],
    queryFn: async ({ pageParam = 1 }) =>
      await queryDataInfinite(
        `${apiVersion}/controllers/dev/settings/notifications/page.php?start=${pageParam}`,
        `${apiVersion}/controllers/dev/settings/notifications/page.php?start=${pageParam}`, // list endpoint
                ...
                {page?.data?.map((item, key) => {
                  return (
                    <tr key={key}>
                      ...
                      <td>{item.notification_updated}</td>
                      <td>
