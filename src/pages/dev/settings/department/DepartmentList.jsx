...
const DepartmentList = ({ setItemEdit, itemEdit }) => {
    ...
    const {
        ...
        queryKey: ["department", searchValue, store.isSearch, filterData],
        queryFn: async ({ pageParam = 1 }) =>
            await queryDataInfinite(
                `${apiVersion}/controllers/dev/settings/department/page.php?start=${pageParam}`,
                `${apiVersion}/controllers/dev/settings/department/page.php?start=${pageParam}`, // list endpoint
    ...
    return (
        <>
                                ...
                                {page?.data?.map((item, key) => {
                                    return (
                                        <tr key={key}>
                                            ...
                                            <td>{item.department_created}</td>
                                            <td>{item.department_updated}</td>
                                            <td>
            ...
            {store.isRestore && (
                <ModalRestore
                    mysqlApiRestore={`${apiVersion}/controllers/dev/settings/department/department.php?id=${itemEdit.department_aid}&action=archive`}
                    dataItem={itemEdit}
                    queryKey={"department"}
                    msg={"Are you sure to restore this record?"}
                    successMsg={"Successfully restored this record"}
                    item={`${itemEdit.department_first_name} ${itemEdit.department_last_name}`}
                />
            )}
            {store.isDelete && (
                <ModalDelete
                    mysqlApiDelete={`${apiVersion}/controllers/dev/settings/department/department.php?id=${itemEdit.department_aid}`}
                    dataItem={itemEdit}
                    queryKey={"department"}
            ...
            {store.isArchive && (
                <ModalArchive
                    mysqlApiArchive={`${apiVersion}/controllers/dev/settings/department/department.php?id=${itemEdit.department_aid}&action=archive`}
                    dataItem={itemEdit}
                    queryKey={"department"}
