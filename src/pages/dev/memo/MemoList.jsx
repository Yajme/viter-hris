...
import { FaTrash, FaNewspaper } from "react-icons/fa6";
import ModalDelete from "#partials/modals/ModalDelete";
import ModalArchive from "#partials/modals/ModalArchive";
import ModalRestore from "#partials/modals/ModalRestore";
import SearchBar from "#partials/SearchBar";

export default function MemoList({ itemEdit, setItemEdit }) {
    ...
    const {
        ...
        queryKey: ["memo", searchValue, store.isSearch, filterData],
        queryFn: async ({ pageParam = 1 }) =>
            await queryDataInfinite(
                `${apiVersion}/controllers/dev/memo/page.php?start=${pageParam}`,
                `${apiVersion}/controllers/dev/memo/page.php?start=${pageParam}`, // list endpoint
    ...
    return (
        <>
            ...
            {store.isRestore && (
                <ModalRestore
                    mysqlApiRestore={`${apiVersion}/controllers/dev/memo/index.php?id=${itemEdit.memo_aid}&action=archive`}
                    dataItem={itemEdit}
                    queryKey={"memo"}
                    msg={"Are you sure to restore this record?"}
                    successMsg={"Successfully restored this record"}
                    item={`${itemEdit.memo_first_name} ${itemEdit.memo_last_name}`}
                />
            )}
            {store.isDelete && (
                <ModalDelete
                    mysqlApiDelete={`${apiVersion}/controllers/dev/memo/index.php?id=${itemEdit.memo_aid}`}
                    dataItem={itemEdit}
                    queryKey={"memo"}
            ...
            {store.isArchive && (
                <ModalArchive
                    mysqlApiArchive={`${apiVersion}/controllers/dev/memo/index.php?id=${itemEdit.memo_aid}&action=archive`}
                    dataItem={itemEdit}
                    queryKey={"memo"}
