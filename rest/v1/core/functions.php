...
function checkReadQuery($query, $total_result, $object_total, $object_start)
{
    ...
    $returnData["total_pages"] = ceil(
        $total_result->rowCount() / $object_total,
    );
...
function checkUpdateEmailForUser($object)
{
    ...
    checkQuery(
        $query,
        "There's a problem processing your request. (update email)",
    );
    return $query;
}
...
function checkUpdateUserKeyAndNewEmail($object)
{
    ...
    checkQuery(
        $query,
        "There's a problem processing your request. (update user key and new email)",
    );
    return $query;
}
...
function checkSetPassword($object)
{
    ...
    checkQuery(
        $query,
        "There's a problem processing your request. (set password)",
    );
    return $query;
}
...
function checkResetPassword($object)
{
    ...
    checkQuery(
        $query,
        "There's a problem processing your request. (reset password)",
    );
    return $query;
}
...
function checkAssociatedById($object)
{
    ...
    checkQuery(
        $query,
        "There's a problem processing your request. (check is associated by id)",
    );
    return $query;
}
...
function checkAssociatedByName($object)
{
    ...
    checkQuery(
        $query,
        "There's a problem processing your request. (check is associated by name)",
    );
    return $query;
}
...
function isAssociated($object)
{
    ...
    checkExistence(
        $count,
        "You cannot delete this item because it is already associated with other module.",
    );
}
...
{
    if (
        strtolower($name_old) != strtolower($name) ||
        strtolower($id_old) != strtolower($id)
    ) {
        isNameExist($object, $name);
    }
}
